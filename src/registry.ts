...
export class BabelRegistryClient {
    ...
    public async registerPrimitive(id: number, template: string, description: string): Promise<RegistrationResult> {
        if (!this.program || !this.provider) {
            console.log(`[Babel-Sim] Registering Primitive ${id}: ${template}`);
            return { success: true, sig: "simulated" };
        }
        ...
        try {
            ...
            return { success: false, error: e.message?.substring(0, 80) || "Unknown error" };
        }
    }
}
